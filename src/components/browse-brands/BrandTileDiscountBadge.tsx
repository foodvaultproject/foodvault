...
export function BrandTileDiscountBadge({
  ...
  if (percent != null) {
    return (
      <div
        className={`absolute right-3 top-3 z-10 rounded-md bg-primary px-3 py-2 text-center text-primary-foreground shadow-[0_4px_14px_rgba(79,70,229,0.28)] ${className}`.trim()}
      >
  ...
  return (
    <div
      className={`absolute right-3 top-3 z-10 max-w-[8.5rem] rounded-md bg-primary px-3 py-2 text-center text-[11px] font-bold leading-tight text-primary-foreground shadow-[0_4px_14px_rgba(79,70,229,0.28)] ${className}`.trim()}
    >
      {discountLabel}
    </div>
  );
