...
type ExploreLightboxImageOverlayProps = {
...
function ExploreLightboxImageOverlay({
  item,
  onClose,
  canFavorite,
  initialFavorited,
}: ExploreLightboxImageOverlayProps) {
  return (
    <div className="pointer-events-none absolute inset-0 flex flex-col justify-between">
      ...
      <div className="pointer-events-auto bg-gradient-to-t from-black/80 via-black/45 to-transparent px-4 pb-[calc(0.85rem+env(safe-area-inset-bottom,0px))] pt-12 md:pb-4">
        <div className="flex items-end justify-between gap-4">
          <Link
            href={`/brands/${item.partnerSlug}`}
            className="flex min-w-0 items-center gap-3 transition-opacity hover:opacity-90"
          >
            <PartnerLogo
              src={item.logoUrl}
              originalSrc={item.logoOriginalUrl}
              alt=""
            ...
            {items.map((item, index) => (
              ...
              >
                <div className="relative w-full">
                  ...
                  />
                  <ExploreLightboxImageOverlay
                    item={item}
                    onClose={close}
                    canFavorite={canFavorite}
                    initialFavorited={favoritedSet.has(item.partnerId)}
                  />
                </div>
              </div>
            ))}
