-- Expose dietary & lifestyle attributes on public brand listings and search.
...
select
  ...
  p.category_groups,
  p.subcategories,
  p.dietary_lifestyle_attributes,
  p.offer_type,
  p.discount_value,
  coalesce(
    p.discount_percent,
...
create or replace function public.search_public_brands(
  ...
  p_dietary_lifestyle text default null,
...
begin
  ...
  with filtered as (
    ...
    where (
      ...
      and (
        p_dietary_lifestyle is null
        or trim(p_dietary_lifestyle) = ''
        or p_dietary_lifestyle = any(coalesce(v.dietary_lifestyle_attributes, '{}'::text[]))
        or exists (
          select 1
