-- Legacy partners table compatibility: older databases require `description`
-- (NOT NULL) and may also use `auth_user_id` alongside `user_id`.

alter table public.partners
  add column if not exists description text,
  add column if not exists auth_user_id uuid;

-- Backfill any existing rows missing legacy fields.
update public.partners
set description = coalesce(
      nullif(trim(short_description), ''),
      nullif(trim(brand_story), ''),
...
-- Keep submit RPC in sync with legacy columns on upsert/resubmit.
