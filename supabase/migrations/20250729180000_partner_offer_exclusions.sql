-- Optional member-offer exclusions text shown on brand profiles when provided.

alter table public.partners
  add column if not exists offer_exclusions text;

comment on column public.partners.offer_exclusions is
  'Optional exclusions for the member offer (max 100 characters).';
...
create or replace function public.submit_partner_application(
  ...
  p_offer_exclusions text default null
)
returns json
language plpgsql
security definer
...
declare
  ...
  v_offer_exclusions text := nullif(left(trim(coalesce(p_offer_exclusions, '')), 100), '');
begin
  ...
  on conflict (user_id) do update set
    ...
    offer_exclusions = excluded.offer_exclusions,
...
grant execute on function public.submit_partner_application(
  text, text, text, text, text, text[], text, text, text, text, text, text, text, text, text, text, text, text, text, text[], text, jsonb, text[], jsonb, text, jsonb, text, jsonb, numeric, boolean, smallint, smallint, text, text, text
) to authenticated;
